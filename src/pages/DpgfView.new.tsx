...
export default function DpgfView() {
  ...
  // Calculer le total général du DPGF
    ...
    <div className="space-y-6 p-4">
      <div className="bg-white p-4 rounded shadow">
        ...
        <div className="mt-2 text-gray-600">
          <p>Client: <span className="font-medium">{dpgfStructure.client?.nom_client}</span></p>
          ...
          <p>Total: <span className="font-medium">{totalGeneral.toFixed(2)} €</span></p>
        </div>
  ...
  return (
    <div className="rounded border bg-white shadow">
      ...
      <div className="p-3 border-t bg-gray-50 flex justify-between items-center">
        ...
        <span className="text-lg font-semibold">{totalLot.toFixed(2)} €</span>
      </div>
    </div>
  );
}
            ...
            <tbody className="bg-white divide-y divide-slate-100">
              {section.elements.map(element => (
                <tr key={element.id_element} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-right">{element.prix_unitaire_ht.toFixed(2)} €</td>
                  <td className="px-3 py-2 text-right font-medium">{element.prix_total_ht.toFixed(2)} €</td>
                </tr>
              ))}
              <tr className="bg-gray-50">
                <td colSpan={4} className="px-3 py-2 text-right font-medium">Total section</td>
                <td className="px-3 py-2 text-right font-semibold">{sectionTotal.toFixed(2)} €</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function ExcelView({ dpgfStructure }: { dpgfStructure: DpgfStructure }) {
  ...
  const grandTotal = useMemo(() => {
    return flatElements.reduce((total, item) => total + (item.element.prix_total_ht || 0), 0);
  }, [flatElements]);
            ...
            {flatElements.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right">{item.element.prix_unitaire_ht.toFixed(2)} €</td>
                  <td className="px-3 py-2 text-right font-medium">{item.element.prix_total_ht.toFixed(2)} €</td>
                </tr>
              );
            })}
            <tr className="bg-gray-100 font-semibold">
              <td colSpan={8} className="px-3 py-3 text-right">TOTAL GÉNÉRAL</td>
              <td className="px-3 py-3 text-right text-lg">{grandTotal.toFixed(2)} €</td>
            </tr>
          </tbody>
        </table>
      </div>
